"""
This file contains tests for running the Elixir Language Server: ElixirLS
"""
...
@pytest.mark.asyncio
async def test_multilspy_elixir_basic_functionality():
    """
    Test basic ElixirLS functionality with a self-contained project
    """
    with create_elixir_test_context() as context:
        ...
        async with lsp.start_server():
            ...
            result = await lsp.request_document_symbols(str(PurePath("lib/test_elixir_project.ex")))
            ...
            if result[0]:  # If we have symbols
                ...
                assert any("add" in name for name in symbol_names)

            # Test 2: Document symbols for math module
            result = await lsp.request_document_symbols(str(PurePath("lib/math.ex")))
            assert isinstance(result, tuple)
            assert len(result) == 2
...
@pytest.mark.asyncio
async def test_multilspy_elixir_definitions():
    """
    Test definition requests for local function calls
    """
    with create_elixir_test_context() as context:
        lsp = LanguageServer.create(context.config, context.logger, context.source_directory)
        ...
        async with lsp.start_server():
            ...
            # Line ~28: Math.multiply(a, b)
            result = await lsp.request_definition(str(PurePath("lib/server.ex")), 27, 10)
            assert isinstance(result, list)

            # Test definition for TestElixirProject.add call
            # Line ~24: TestElixirProject.add(a, b)
            result = await lsp.request_definition(str(PurePath("lib/server.ex")), 23, 20)
            assert isinstance(result, list)

            # Test definition for Math.square call
            # Line ~32: Math.square(n)
            result = await lsp.request_definition(str(PurePath("lib/server.ex")), 31, 10)
            assert isinstance(result, list)


@pytest.mark.asyncio
async def test_multilspy_elixir_hover():
    """
    Test hover information for functions and modules
    """
    with create_elixir_test_context() as context:
        lsp = LanguageServer.create(context.config, context.logger, context.source_directory)
        ...
        async with lsp.start_server():
            ...
            result = await lsp.request_hover(str(PurePath("lib/test_elixir_project.ex")), 0, 8)
            ...
            result = await lsp.request_hover(str(PurePath("lib/test_elixir_project.ex")), 0, 20)
            assert isinstance(result, dict), "Should return hover info for module name"

            # Test hover on :world atom - should return hover info
            result = await lsp.request_hover(str(PurePath("lib/test_elixir_project.ex")), 8, 4)
            assert isinstance(result, dict), "Should return hover info for atom"


@pytest.mark.asyncio
async def test_multilspy_elixir_hover_none_cases():
    """
    ...
    with create_elixir_test_context() as context:
        ...
        async with lsp.start_server():
            ...
            result = await lsp.request_hover(str(PurePath("lib/test_elixir_project.ex")), 7, 6)
            ...
            result = await lsp.request_hover(str(PurePath("lib/test_elixir_project.ex")), 14, 4)
            assert result is None, "Should return None for parameter name"

            # Test hover on whitespace - should return None
            result = await lsp.request_hover(str(PurePath("lib/test_elixir_project.ex")), 1, 0)
            assert result is None, "Should return None for whitespace position"


@pytest.mark.asyncio
async def test_multilspy_elixir_workspace_symbols():
    """
    ...
    with create_elixir_test_context() as context:
        ...
        async with lsp.start_server():
            ...
            result = await lsp.request_workspace_symbol("TestElixirProject")
            assert isinstance(result, list)

            # Test searching for functions
            result = await lsp.request_workspace_symbol("multiply")
            assert isinstance(result, list)

            # Test empty search (should return all symbols if any)
            result = await lsp.request_workspace_symbol("")
            assert isinstance(result, list)


@pytest.mark.asyncio
async def test_multilspy_elixir_references():
    """
    Test finding references to functions and modules
    """
    with create_elixir_test_context() as context:
        lsp = LanguageServer.create(context.config, context.logger, context.source_directory)
        ...
        async with lsp.start_server():
            ...
            result = await lsp.request_references(str(PurePath("lib/math.ex")), 6, 6)
            assert isinstance(result, list)
            ...
            # Test references for hello function
            result = await lsp.request_references(str(PurePath("lib/test_elixir_project.ex")), 6, 6)
            assert isinstance(result, list)


@pytest.mark.asyncio
async def test_multilspy_elixir_multiple_files():
    """
    Test functionality across multiple files
    """
    with create_elixir_test_context() as context:
        lsp = LanguageServer.create(context.config, context.logger, context.source_directory)
        ...
        async with lsp.start_server():
            ...
            for file_path in files_to_test:
                result = await lsp.request_document_symbols(str(PurePath(file_path)))
                assert isinstance(result, tuple)
                assert len(result) == 2
            ...
            assert any("Server" in symbol for symbol in all_symbols)


@pytest.mark.asyncio
async def test_multilspy_elixir_error_handling():
    """
    Test ElixirLS error handling and edge cases
    """
    with create_elixir_test_context() as context:
        lsp = LanguageServer.create(context.config, context.logger, context.source_directory)
        ...
        async with lsp.start_server():
            ...
            try:
                result = await lsp.request_definition("lib/nonexistent.ex", 1, 1)
            ...
            result = await lsp.request_definition(str(PurePath("lib/test_elixir_project.ex")), 99999, 99999)
            assert isinstance(result, list)

            # Test hover on edge case position - should return None
            result = await lsp.request_hover(str(PurePath("lib/test_elixir_project.ex")), 99999, 99999)
